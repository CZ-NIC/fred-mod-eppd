...
typedef enum { CMD_UNKNOWN, CMD_CUSTOM, CMD_FILE, CMD_EXIT } cmd_t;
...
cmd_t getcmd(void)
{
    ...
    if (!strncmp("custom", cmd, 30))
        return CMD_CUSTOM;
    if (!strncmp("file", cmd, 30))
        return CMD_FILE;
    if (!strncmp("exit", cmd, 30))
        return CMD_EXIT;
    return CMD_UNKNOWN;
}
...
int main(int argc, char *argv[])
{
    ...
    while (!quit)
    {
        ...
        if (cstat != CORBA_INT_ERROR)
        {
            ...
            gstat = epp_gen_response(&epp_ctx, 1, schema, lang, cdata, &response, &valerr);
            switch (gstat)
            {
                /*
            ...
            fputs("Internal error in Corba part\n", stderr);
