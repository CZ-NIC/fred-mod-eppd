        ...
        ccReg_TID *const log_session_id, char *errmsg)
{
    ...
    {
        if (retr != 0)
            CORBA_exception_free(ev); // valid first time
    ...
    *log_session_id = session_id;
    epplog(epp_ctx,
           EPP_INFO,
           "Created session in fred-logd with id: %" APR_UINT64_T_FMT,
           *log_session_id);
        ...
        epp_action_type action_type, ccReg_TID *log_entry_id, ccReg_TID sessionid, char *errmsg)
{
    ...
    for (retr = 0; retr < MAX_RETRIES; retr++)
    {
        ...
        /* call logger method */
        entry_id = ccReg_Logger_createRequest(
                (ccReg_Logger)service,
                c_source_ip,
                LC_EPP,
                c_content,
                properties,
...
static epp_action_type log_props_create(ccReg_RequestProperties **c_props, epp_command_data *cdata)
{
    ...
    switch (cdata->type)
    {
        case EPP_CREATE_CONTACT:
            ...
            {
                case ident_UNKNOWN:
                    PUSH_PROPERTY(*c_props, "identType", "unknown");
                    break;
                case ident_OP:
                    PUSH_PROPERTY(*c_props, "identType", "ID card");
                    break;
                case ident_PASSPORT:
                ...
                case ident_ICO:
                    ...
                    break;
                case ident_BIRTHDAY:
                    PUSH_PROPERTY(*c_props, "identType", "birthdate");
                    break;
            }
        ...
        const epp_command_data *cdata, ccReg_TID session_id, ccReg_TID log_entry_id)
{
    ...
    if (cdata != NULL)
    {
        ...
        if (cdata->type == EPP_CHECK_CONTACT || cdata->type == EPP_CHECK_DOMAIN ||
            cdata->type == EPP_CHECK_NSSET || cdata->type == EPP_CHECK_KEYSET)
        {
