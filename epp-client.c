		...
		epp_command_data *cdata)
{
	...
	c_params = init_epp_params(loginid, log_id, cdata->xml_in, cdata->clTRID);
		...
		epp_command_data *cdata)
{
	...
	switch (cdata->type) {
		...
		case EPP_SENDAUTHINFO_DOMAIN:
			cdata->noresdata = 1;
			cstat = epp_call_sendauthinfo(epp_ctx, service, loginid, log_id,
					cdata, EPP_DOMAIN);
			break;
