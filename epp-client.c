...
int epp_call_hello(epp_context *epp_ctx, service_EPP service, char **version, char **curdate)
{
    ...
    for (retr = 0; retr < MAX_RETRIES; retr++)
    {
        ...
        c_version = ccReg_EPP_version((ccReg_EPP)service, &c_curdate, ev);
...
epilog_success(epp_context *epp_ctx, epp_command_data *cdata, ccReg_Response *response)
{
    ...
    cdata->svTRID = unwrap_str_req(epp_ctx, response->svTRID, &cerrno, "svTRID");
    CORBA_free(response);
...
static int epilog_failure(epp_context *epp_ctx, epp_command_data *cdata, ccReg_EPP_EppError *exc)
{
    ...
    for (i = 0; i < exc->errorList._length; i++)
    {
        ...
        if (err_item == NULL)
            return 1;
        err_item->reason = unwrap_str_req(epp_ctx, c_error->reason, &cerrno, "reason");
        if (cerrno != 0)
            return cerrno;
        /* copy position spec */
        ...
        const ccReg_TID request_id, epp_command_data *cdata)
{
    ...
    for (retr = 0; retr < MAX_RETRIES; retr++)
    {
        ...
        response = ccReg_EPP_GetTransaction(
                (ccReg_EPP)service,
                cdata->rc,
                loginid,
                request_id,
    ...
    q_foreach(&cdata->errors)
    {
        ...
        if (err_item->spec == errspec_not_valid)
            err_item->reason =
                    epp_strcat(epp_ctx->pool, c_errStrings->_buffer[i++], err_item->reason);
        else
            err_item->reason = unwrap_str(epp_ctx->pool, c_errStrings->_buffer[i++], &cerrno);
        ...
        const ccReg_TID request_id, epp_lang *lang, const char *certID, epp_command_data *cdata)
{
    ...
    for (retr = 0; retr < MAX_RETRIES; retr++)
    {
        ...
        response = ccReg_EPP_ClientLogin(
                (ccReg_EPP)service,
                c_clID,
                c_pw,
                c_newPW,
        ...
        const ccReg_TID request_id, epp_command_data *cdata)
{
    ...
    for (retr = 0; retr < MAX_RETRIES; retr++)
    {
        ...
        response = ccReg_EPP_ClientLogout((ccReg_EPP)service, c_params, ev);
        ...
        const ccReg_TID request_id, epp_command_data *cdata, epp_object_type obj)
{
    ...
    for (retr = 0; retr < MAX_RETRIES; retr++)
    {
        ...
        if (obj == EPP_CONTACT)
            response = ccReg_EPP_ContactCheck((ccReg_EPP)service, c_ids, &c_avails, c_params, ev);
        else if (obj == EPP_DOMAIN)
            response = ccReg_EPP_DomainCheck((ccReg_EPP)service, c_ids, &c_avails, c_params, ev);
        else if (obj == EPP_NSSET)
        {
            response = ccReg_EPP_NSSetCheck((ccReg_EPP)service, c_ids, &c_avails, c_params, ev);
        }
        else
        {
            assert(obj == EPP_KEYSET);
            response = ccReg_EPP_KeySetCheck((ccReg_EPP)service, c_ids, &c_avails, c_params, ev);
        }
    ...
    for (i = 0; i < c_avails->_length; i++)
    {
        ...
        avail->reason = unwrap_str(epp_ctx->pool, c_avails->_buffer[i].reason, &cerrno);
        if (cerrno != 0)
            break;
        ...
        if (!avail->avail && avail->reason == NULL)
        {
            epplog(epp_ctx,
                   EPP_ERROR,
                   "Reason is empty and object "
                   "is not available");
            CORBA_free(c_avails);
            CORBA_free(response);
            return create_dummy_answer(epp_ctx, cdata);
        }
        if (q_add(epp_ctx->pool, &check->avails, avail))
            break;
    }
        ...
        epp_command_data *cdata)
{
    ...
    if (has_contact_mailing_address_extension && c_contact->MailingAddress.is_set)
    {
        ...
        ext_item->ext.ext_mailing_addr.data.info.StateOrProvince = unwrap_str_req(
                epp_ctx,
                c_contact->MailingAddress.data.StateOrProvince,
                &cerrno,
                "stateorprovince");
        if (cerrno != 0)
        {
        ...
        ext_item->ext.ext_mailing_addr.data.info.PostalCode = unwrap_str_req(
                epp_ctx, c_contact->MailingAddress.data.PostalCode, &cerrno, "postalcode");
        if (cerrno != 0)
        {
            goto error;
        }
        ext_item->ext.ext_mailing_addr.data.info.CountryCode = unwrap_str_req(
                epp_ctx, c_contact->MailingAddress.data.CountryCode, &cerrno, "countrycode");
        if (cerrno != 0)
        {
            goto error;
        }
        ...
        CORBA_Environment *ev)
{
    ...
    /* copy output values */
    info_domain->roid = unwrap_str_req(epp_ctx, c_domain->ROID, &cerrno, "ROID");
    if (cerrno != 0)
        goto error;
    info_domain->handle = unwrap_str_req(epp_ctx, c_domain->name, &cerrno, "handle");
    if (cerrno != 0)
        goto error;
    info_domain->clID = unwrap_str_req(epp_ctx, c_domain->ClID, &cerrno, "clID");
    if (cerrno != 0)
        goto error;
    info_domain->crID = unwrap_str_req(epp_ctx, c_domain->CrID, &cerrno, "crID");
    if (cerrno != 0)
        goto error;
    info_domain->upID = unwrap_str(epp_ctx->pool, c_domain->UpID, &cerrno);
    if (cerrno != 0)
        goto error;
    info_domain->crDate = unwrap_str_req(epp_ctx, c_domain->CrDate, &cerrno, "crDate");
    if (cerrno != 0)
        goto error;
    info_domain->upDate = unwrap_str(epp_ctx->pool, c_domain->UpDate, &cerrno);
    if (cerrno != 0)
        goto error;
    info_domain->trDate = unwrap_str(epp_ctx->pool, c_domain->TrDate, &cerrno);
    if (cerrno != 0)
        goto error;
    info_domain->exDate = unwrap_str(epp_ctx->pool, c_domain->ExDate, &cerrno);
    if (cerrno != 0)
        goto error;
    info_domain->registrant = unwrap_str(epp_ctx->pool, c_domain->Registrant, &cerrno);
    if (cerrno != 0)
        goto error;
    info_domain->nsset = unwrap_str(epp_ctx->pool, c_domain->nsset, &cerrno);
    if (cerrno != 0)
        goto error;
    info_domain->keyset = unwrap_str(epp_ctx->pool, c_domain->keyset, &cerrno);
    if (cerrno != 0)
        goto error;
    info_domain->authInfo = unwrap_str(epp_ctx->pool, c_domain->AuthInfoPw, &cerrno);
    if (cerrno != 0)
        goto error;
    ...
    for (i = 0; i < c_domain->stat._length; i++)
    {
        ...
        status->value =
                unwrap_str_req(epp_ctx, c_domain->stat._buffer[i].value, &cerrno, "status flag");
        if (cerrno != 0)
            goto error;
        status->text =
                unwrap_str_req(epp_ctx, c_domain->stat._buffer[i].text, &cerrno, "status text");
        if (cerrno != 0)
            goto error;
        ...
        CORBA_Environment *ev)
{
    ...
    /* copy output values */
    info_nsset->roid = unwrap_str_req(epp_ctx, c_nsset->ROID, &cerrno, "ROID");
    if (cerrno != 0)
        goto error;
    info_nsset->handle = unwrap_str_req(epp_ctx, c_nsset->handle, &cerrno, "handle");
    if (cerrno != 0)
        goto error;
    info_nsset->clID = unwrap_str_req(epp_ctx, c_nsset->ClID, &cerrno, "clID");
    if (cerrno != 0)
        goto error;
    info_nsset->crID = unwrap_str_req(epp_ctx, c_nsset->CrID, &cerrno, "crID");
    if (cerrno != 0)
        goto error;
    info_nsset->upID = unwrap_str(epp_ctx->pool, c_nsset->UpID, &cerrno);
    if (cerrno != 0)
        goto error;
    info_nsset->crDate = unwrap_str_req(epp_ctx, c_nsset->CrDate, &cerrno, "crDate");
    if (cerrno != 0)
        goto error;
    ...
    if (cerrno != 0)
        goto error;
    info_nsset->authInfo = unwrap_str(epp_ctx->pool, c_nsset->AuthInfoPw, &cerrno);
    if (cerrno != 0)
        goto error;
    ...
    for (i = 0; i < c_nsset->stat._length; i++)
    {
        ...
        status->value =
                unwrap_str_req(epp_ctx, c_nsset->stat._buffer[i].value, &cerrno, "status flag");
        if (cerrno != 0)
            goto error;
        status->text =
                unwrap_str_req(epp_ctx, c_nsset->stat._buffer[i].text, &cerrno, "status text");
        if (cerrno != 0)
            goto error;
    ...
    for (i = 0; i < c_nsset->dns._length; i++)
    {
        ...
        /* process of ns item */
        ns_item->name = unwrap_str_req(epp_ctx, c_nsset->dns._buffer[i].fqdn, &cerrno, "fqdn");
        if (cerrno != 0)
            goto error;
        ...
        CORBA_Environment *ev)
{
    ...
    /* copy output values */
    info_keyset->roid = unwrap_str_req(epp_ctx, c_keyset->ROID, &cerrno, "ROID");
    if (cerrno != 0)
        goto error;
    info_keyset->handle = unwrap_str_req(epp_ctx, c_keyset->handle, &cerrno, "handle");
    if (cerrno != 0)
        goto error;
    info_keyset->clID = unwrap_str_req(epp_ctx, c_keyset->ClID, &cerrno, "clID");
    if (cerrno != 0)
        goto error;
    info_keyset->crID = unwrap_str_req(epp_ctx, c_keyset->CrID, &cerrno, "crID");
    if (cerrno != 0)
        goto error;
    info_keyset->upID = unwrap_str(epp_ctx->pool, c_keyset->UpID, &cerrno);
    if (cerrno != 0)
        goto error;
    info_keyset->crDate = unwrap_str_req(epp_ctx, c_keyset->CrDate, &cerrno, "crDate");
    if (cerrno != 0)
        goto error;
    ...
    if (cerrno != 0)
        goto error;
    info_keyset->authInfo = unwrap_str(epp_ctx->pool, c_keyset->AuthInfoPw, &cerrno);
    if (cerrno != 0)
        goto error;
    ...
    for (i = 0; i < c_keyset->stat._length; i++)
    {
        ...
        status->value =
                unwrap_str_req(epp_ctx, c_keyset->stat._buffer[i].value, &cerrno, "status flag");
        if (cerrno != 0)
            goto error;
        status->text =
                unwrap_str_req(epp_ctx, c_keyset->stat._buffer[i].text, &cerrno, "status text");
        if (cerrno != 0)
            goto error;
    ...
    for (i = 0; i < c_keyset->dnsk._length; i++)
    {
        ...
        dnskey_item->public_key =
                unwrap_str_req(epp_ctx, c_keyset->dnsk._buffer[i].key, &cerrno, "public_key");
        if (cerrno != 0)
            goto error;
        ...
        const ccReg_TID request_id, epp_command_data *cdata)
{
    ...
    for (retr = 0; retr < MAX_RETRIES; retr++)
    {
        ...
        response = ccReg_EPP_PollRequest(
                (ccReg_EPP)service,
                &c_msgID,
                &c_count,
                &c_qdate,
    ...
    switch (c_polltype)
    {
        case ccReg_polltype_transfer_domain:
        {
            ...
            if (cerrno != 0)
                goto error;
            poll_req->msg.hdt.date = unwrap_str(epp_ctx->pool, hdr->date, &cerrno);
            if (cerrno != 0)
                goto error;
            poll_req->msg.hdt.clID = unwrap_str(epp_ctx->pool, hdr->clID, &cerrno);
            if (cerrno != 0)
                goto error;
            break;
        }
        ...
        case ccReg_polltype_delete_domain:
        {
            ...
            if (cerrno != 0)
                goto error;
            poll_req->msg.hd.date = unwrap_str(epp_ctx->pool, hd->date, &cerrno);
            if (cerrno != 0)
                goto error;
            break;
        }
        ...
        case ccReg_polltype_techcheck:
        {
            ...
            for (i = 0; i < tc->tests._length; i++)
            {
                ...
                if (cerrno != 0)
                    goto error;
                tr->note = unwrap_str(epp_ctx->pool, tci->note, &cerrno);
                if (cerrno != 0)
                    goto error;
                if (q_add(epp_ctx->pool, &poll_req->msg.tc.tests, tr))
                    goto error;
            }
            break;
        }
        ...
        case ccReg_polltype_request_fee_info:
        {
            ...
            if (cerrno != 0)
                goto error;
            poll_req->msg.rfi.period_to = unwrap_str(epp_ctx->pool, rfi->periodTo, &cerrno);
            if (cerrno != 0)
                goto error;
        ...
        case ccReg_polltype_update_domain:
        {
            ...
            ccReg_EPP_PollRequestGetUpdateDomainDetails(
            ...
            if (info_domain_data_copy(epp_ctx, &poll_req->msg.upd.old_data, c_old_data, ev) != 1)
            {
                CORBA_free(c_old_data);
                CORBA_free(c_new_data);
                goto error;
            }
            if (info_domain_data_copy(epp_ctx, &poll_req->msg.upd.new_data, c_new_data, ev) != 1)
            {
                CORBA_free(c_old_data);
                CORBA_free(c_new_data);
                goto error;
        ...
        case ccReg_polltype_update_nsset:
        {
            ...
            ccReg_EPP_PollRequestGetUpdateNSSetDetails(
                    (ccReg_EPP)service, up->pollID, &c_old_data, &c_new_data, c_params, ev);
        ...
        case ccReg_polltype_update_keyset:
        {
            ...
            ccReg_EPP_PollRequestGetUpdateKeySetDetails(
                    (ccReg_EPP)service, up->pollID, &c_old_data, &c_new_data, c_params, ev);
        ...
        const ccReg_TID request_id, epp_command_data *cdata)
{
    ...
    for (retr = 0; retr < MAX_RETRIES; retr++)
    {
        ...
        /* send acknoledgement */
        response = ccReg_EPP_PollAcknowledgement(
                (ccReg_EPP)service, poll_ack->msgid, &c_count, &c_msgID, c_params, ev);
        ...
        const ccReg_TID request_id, epp_command_data *cdata)
{
    ...
    if (len != 0 && c_ext_list->_buffer == NULL)
        goto error_input;
    ...
    q_foreach(&create_domain->extensions)
    {
        ...
        if (ext_item->extType == EPP_EXT_ENUMVAL)
        {
            ...
            if (c_enumval == NULL)
                goto error_input;
            c_enumval->valExDate = wrap_str(ext_item->ext.ext_enum.ext_enumval);
    ...
    for (retr = 0; retr < MAX_RETRIES; retr++)
    {
        ...
        response = ccReg_EPP_DomainCreate(
                (ccReg_EPP)service,
                create_domain->name,
                c_registrant,
                c_nsset,
...
error_input:
    ...
    create_domain->crDate = unwrap_str_req(epp_ctx, c_crDate, &cerrno, "crDate");
    if (cerrno != 0)
        goto error;
    create_domain->exDate = unwrap_str_req(epp_ctx, c_exDate, &cerrno, "exDate");
    if (cerrno != 0)
        goto error;
        ...
        int has_contact_mailing_address_extension, epp_command_data *cdata)
{
    ...
    c_contact->NotifyEmail = wrap_str(create_contact->notify_email);
    ...
    for (retr = 0; retr < MAX_RETRIES; retr++)
    {
        ...
        response = ccReg_EPP_ContactCreate(
                (ccReg_EPP)service, create_contact->id, c_contact, &c_crDate, c_params, ev);
        ...
        const ccReg_TID request_id, epp_command_data *cdata)
{
    ...
    q_foreach(&create_nsset->ns)
    {
        ...
        c_dnshost->_buffer[i].inet._maximum = c_dnshost->_buffer[i].inet._length = len;
        c_dnshost->_buffer[i].inet._release = CORBA_TRUE;
        j = 0;
        q_foreach(&ns->addr)
        {
            c_dnshost->_buffer[i].inet._buffer[j] = wrap_str(q_content(&ns->addr));
    ...
    for (retr = 0; retr < MAX_RETRIES; retr++)
    {
        ...
        response = ccReg_EPP_NSSetCreate(
                (ccReg_EPP)service,
                create_nsset->id,
                c_authInfo,
                c_tech,
        ...
        const ccReg_TID request_id, epp_command_data *cdata)
{
    ...
    for (retr = 0; retr < MAX_RETRIES; retr++)
    {
        ...
        response = ccReg_EPP_KeySetCreate(
                (ccReg_EPP)service,
                create_keyset->id,
                c_authInfo,
                c_tech,
        ...
        const ccReg_TID request_id, epp_command_data *cdata, epp_object_type obj)
{
    ...
    for (retr = 0; retr < MAX_RETRIES; retr++)
    {
        ...
        if (obj == EPP_DOMAIN)
            response = ccReg_EPP_DomainDelete((ccReg_EPP)service, delete->id, c_params, ev);
        else if (obj == EPP_CONTACT)
            response = ccReg_EPP_ContactDelete((ccReg_EPP)service, delete->id, c_params, ev);
        else if (obj == EPP_KEYSET)
            response = ccReg_EPP_KeySetDelete((ccReg_EPP)service, delete->id, c_params, ev);
        else
        {
            assert(obj == EPP_NSSET);
            response = ccReg_EPP_NSSetDelete((ccReg_EPP)service, delete->id, c_params, ev);
        }
        ...
        const ccReg_TID request_id, epp_command_data *cdata)
{
    ...
    c_period->unit = (renew->unit == TIMEUNIT_MONTH) ? ccReg_unit_month : ccReg_unit_year;
    /* fill extension list */
    c_ext_list = ccReg_ExtensionList__alloc();
    if (c_ext_list == NULL)
        goto error_input;
    ...
    for (retr = 0; retr < MAX_RETRIES; retr++)
    {
        ...
        response = ccReg_EPP_DomainRenew(
                (ccReg_EPP)service,
                renew->name,
                c_exDateIN,
                c_period,
        ...
        const ccReg_TID request_id, epp_command_data *cdata)
{
    ...
    if (len != 0 && c_admin_add->_buffer == NULL)
        goto error_input;
    ...
    if (len != 0 && c_admin_rem->_buffer == NULL)
        goto error_input;
    ...
    for (retr = 0; retr < MAX_RETRIES; retr++)
    {
        ...
        response = ccReg_EPP_DomainUpdate(
                (ccReg_EPP)service,
                update_domain->name,
                c_registrant,
                c_authInfo,
        ...
        epp_command_data *cdata)
{
    ...
    if (update_contact->pi != NULL)
    {
        ...
        {
            c_contact->Streets._buffer[i] = wrap_str(q_content(&update_contact->pi->streets));
    ...
    if ((c_contact->Name == NULL) || (c_contact->Organization == NULL) ||
        (c_contact->City == NULL) || (c_contact->StateOrProvince == NULL) ||
        (c_contact->PostalCode == NULL) || (c_contact->CC == NULL))
    {
        goto error_input;
    }
    ...
    for (retr = 0; retr < MAX_RETRIES; ++retr)
    {
        ...
        /* send the updates to repository */
        response = ccReg_EPP_ContactUpdate(
                (ccReg_EPP)service, update_contact->id, c_contact, c_params, ev);
        ...
        const ccReg_TID request_id, epp_command_data *cdata)
{
    ...
    if (len != 0 && c_tech_add->_buffer == NULL)
        goto error_input;
    ...
    if (len != 0 && c_tech_rem->_buffer == NULL)
        goto error_input;
    ...
    q_foreach(&update_nsset->add_ns)
    {
        ...
        if (c_dnshost_add->_buffer[i].fqdn == NULL)
            goto error_input;
        ...
        if (len != 0 && c_dnshost_add->_buffer[i].inet._buffer == NULL)
            goto error_input;
        c_dnshost_add->_buffer[i].inet._maximum = c_dnshost_add->_buffer[i].inet._length = len;
    ...
    for (retr = 0; retr < MAX_RETRIES; retr++)
    {
        ...
        /* send the updates to repository */
        response = ccReg_EPP_NSSetUpdate(
                (ccReg_EPP)service,
                update_nsset->id,
                c_authInfo,
                c_dnshost_add,
        ...
        const ccReg_TID request_id, epp_command_data *cdata)
{
    ...
    for (retr = 0; retr < MAX_RETRIES; retr++)
    {
        ...
        /* send the updates to repository */
        response = ccReg_EPP_KeySetUpdate(
                (ccReg_EPP)service,
                update_keyset->id,
                c_authInfo,
                c_tech_add,
        ...
        const ccReg_TID request_id, epp_command_data *cdata, epp_object_type obj)
{
    ...
    for (retr = 0; retr < MAX_RETRIES; retr++)
    {
        ...
        if (obj == EPP_DOMAIN)
        {
            response = ccReg_EPP_DomainTransfer(
                    (ccReg_EPP)service, transfer->id, c_authInfo, c_params, ev);
        }
        else if (obj == EPP_CONTACT)
        {
            response = ccReg_EPP_ContactTransfer(
                    (ccReg_EPP)service, transfer->id, c_authInfo, c_params, ev);
        }
        else if (obj == EPP_KEYSET)
        {
            response = ccReg_EPP_KeySetTransfer(
                    (ccReg_EPP)service, transfer->id, c_authInfo, c_params, ev);
        }
        else
        {
            assert(obj == EPP_NSSET);
            response = ccReg_EPP_NSSetTransfer(
                    (ccReg_EPP)service, transfer->id, c_authInfo, c_params, ev);
        }
        ...
        const ccReg_TID request_id, epp_command_data *cdata, epp_object_type obj)
{
    ...
    for (retr = 0; retr < MAX_RETRIES; retr++)
    {
        ...
        if (obj == EPP_DOMAIN)
        {
            response = ccReg_EPP_DomainList((ccReg_EPP)service, &c_handles, c_params, ev);
        }
        else if (obj == EPP_CONTACT)
        {
            response = ccReg_EPP_ContactList((ccReg_EPP)service, &c_handles, c_params, ev);
        }
        else if (obj == EPP_KEYSET)
        {
            response = ccReg_EPP_KeySetList((ccReg_EPP)service, &c_handles, c_params, ev);
        }
        else
        {
            assert(obj == EPP_NSSET);
            response = ccReg_EPP_NSSetList((ccReg_EPP)service, &c_handles, c_params, ev);
        }
    ...
    for (i = 0; i < c_handles->_length; i++)
    {
        ...
        handle = unwrap_str(epp_ctx->pool, c_handles->_buffer[i], &cerrno);
        if (cerrno != 0)
        {
            CORBA_free(response);
            return CORBA_INT_ERROR;
        ...
        const ccReg_TID request_id, epp_command_data *cdata, epp_object_type obj)
{
    ...
    for (retr = 0; retr < MAX_RETRIES; retr++)
    {
        ...
        if (obj == EPP_DOMAIN)
        {
            response = ccReg_EPP_domainSendAuthInfo((ccReg_EPP)service, c_handle, c_params, ev);
        }
        else if (obj == EPP_CONTACT)
        {
            response = ccReg_EPP_contactSendAuthInfo((ccReg_EPP)service, c_handle, c_params, ev);
        }
        else if (obj == EPP_KEYSET)
        {
            response = ccReg_EPP_keysetSendAuthInfo((ccReg_EPP)service, c_handle, c_params, ev);
        }
        else
        {
            assert(obj == EPP_NSSET);
            response = ccReg_EPP_nssetSendAuthInfo((ccReg_EPP)service, c_handle, c_params, ev);
        }
        ...
        const ccReg_TID request_id, epp_command_data *cdata)
{
    ...
    for (retr = 0; retr < MAX_RETRIES; retr++)
    {
        ...
        response = ccReg_EPP_ClientCredit((ccReg_EPP)service, &c_zoneCredit, c_params, ev);
    ...
    for (i = 0; i < c_zoneCredit->_length; i++)
    {
        ...
        if (zonecredit == NULL)
            break;
        CLEAR_CERRNO(cerrno);
        zonecredit->zone = unwrap_str(epp_ctx->pool, c_zoneCredit->_buffer[i].zone_fqdn, &cerrno);
        if (cerrno != 0)
            break;
        zonecredit->credit = unwrap_str(epp_ctx->pool, c_zoneCredit->_buffer[i].price, &cerrno);
        if (cerrno != 0)
            break;
        ...
        const ccReg_TID request_id, epp_command_data *cdata)
{
    ...
    for (retr = 0; retr < MAX_RETRIES; retr++)
    {
        ...
        response = ccReg_EPP_nssetTest(
                (ccReg_EPP)service, c_handle, test->level, c_names, c_params, ev);
        ...
        const ccReg_TID request_id, epp_command_data *cdata, ccReg_InfoType c_infotype)
{
    ...
    for (retr = 0; retr < MAX_RETRIES; retr++)
    {
        ...
        response = ccReg_EPP_info((ccReg_EPP)service, c_infotype, c_handle, &c_count, c_params, ev);
        ...
        const ccReg_TID request_id, epp_command_data *cdata)
{
    ...
    for (retr = 0; retr < MAX_RETRIES; retr++)
    {
        ...
        response = ccReg_EPP_getInfoResults((ccReg_EPP)service, &c_handles, c_params, ev);
        ...
        epp_command_data *cdata)
{
    ...
    switch (cdata->type)
    {
        ...
        case EPP_CHECK_CONTACT:
            cstat = epp_call_check(epp_ctx, service, loginid, request_id, cdata, EPP_CONTACT);
            break;
        case EPP_CHECK_DOMAIN:
            cstat = epp_call_check(epp_ctx, service, loginid, request_id, cdata, EPP_DOMAIN);
            break;
        case EPP_CHECK_NSSET:
            cstat = epp_call_check(epp_ctx, service, loginid, request_id, cdata, EPP_NSSET);
            break;
        case EPP_CHECK_KEYSET:
            cstat = epp_call_check(epp_ctx, service, loginid, request_id, cdata, EPP_KEYSET);
            break;
        case EPP_INFO_CONTACT:
            cstat = epp_call_info_contact(
                    epp_ctx,
                    service,
                    loginid,
                    request_id,
        ...
        case EPP_INFO_DOMAIN:
            cstat = epp_call_info_domain(epp_ctx, service, loginid, request_id, cdata);
            break;
        case EPP_INFO_NSSET:
            cstat = epp_call_info_nsset(epp_ctx, service, loginid, request_id, cdata);
            break;
        case EPP_INFO_KEYSET:
            cstat = epp_call_info_keyset(epp_ctx, service, loginid, request_id, cdata);
            break;
        case EPP_LIST_CONTACT:
            cstat = epp_call_list(epp_ctx, service, loginid, request_id, cdata, EPP_CONTACT);
            break;
        case EPP_LIST_DOMAIN:
            cstat = epp_call_list(epp_ctx, service, loginid, request_id, cdata, EPP_DOMAIN);
            break;
        case EPP_LIST_NSSET:
            cstat = epp_call_list(epp_ctx, service, loginid, request_id, cdata, EPP_NSSET);
            break;
        case EPP_LIST_KEYSET:
            cstat = epp_call_list(epp_ctx, service, loginid, request_id, cdata, EPP_KEYSET);
            break;
        case EPP_POLL_REQ:
            cdata->noresdata = 1;
        ...
        case EPP_POLL_ACK:
            cdata->noresdata = 1;
            cstat = epp_call_poll_ack(epp_ctx, service, loginid, request_id, cdata);
            break;
        case EPP_CREATE_CONTACT:
            cstat = epp_call_create_contact(
                    epp_ctx,
                    service,
                    loginid,
                    request_id,
        ...
        case EPP_CREATE_DOMAIN:
            cstat = epp_call_create_domain(epp_ctx, service, loginid, request_id, cdata);
            break;
        case EPP_CREATE_NSSET:
            cstat = epp_call_create_nsset(epp_ctx, service, loginid, request_id, cdata);
            break;
        case EPP_CREATE_KEYSET:
            cstat = epp_call_create_keyset(epp_ctx, service, loginid, request_id, cdata);
            break;
        case EPP_DELETE_CONTACT:
            cdata->noresdata = 1;
            cstat = epp_call_delete(epp_ctx, service, loginid, request_id, cdata, EPP_CONTACT);
            break;
        case EPP_DELETE_DOMAIN:
            cdata->noresdata = 1;
            cstat = epp_call_delete(epp_ctx, service, loginid, request_id, cdata, EPP_DOMAIN);
            break;
        case EPP_DELETE_NSSET:
            cdata->noresdata = 1;
            cstat = epp_call_delete(epp_ctx, service, loginid, request_id, cdata, EPP_NSSET);
            break;
        case EPP_DELETE_KEYSET:
            cdata->noresdata = 1;
            cstat = epp_call_delete(epp_ctx, service, loginid, request_id, cdata, EPP_KEYSET);
            break;
        case EPP_RENEW_DOMAIN:
            cstat = epp_call_renew_domain(epp_ctx, service, loginid, request_id, cdata);
            break;
        case EPP_UPDATE_DOMAIN:
            cdata->noresdata = 1;
            cstat = epp_call_update_domain(epp_ctx, service, loginid, request_id, cdata);
            break;
        case EPP_UPDATE_CONTACT:
            cdata->noresdata = 1;
            cstat = epp_call_update_contact(
                    epp_ctx,
                    service,
                    loginid,
                    request_id,
        ...
        case EPP_UPDATE_NSSET:
            cdata->noresdata = 1;
            cstat = epp_call_update_nsset(epp_ctx, service, loginid, request_id, cdata);
            break;
        case EPP_UPDATE_KEYSET:
            cdata->noresdata = 1;
            cstat = epp_call_update_keyset(epp_ctx, service, loginid, request_id, cdata);
            break;

        case EPP_TRANSFER_CONTACT:
            cdata->noresdata = 1;
            cstat = epp_call_transfer(epp_ctx, service, loginid, request_id, cdata, EPP_CONTACT);
            break;
        case EPP_TRANSFER_DOMAIN:
            cdata->noresdata = 1;
            cstat = epp_call_transfer(epp_ctx, service, loginid, request_id, cdata, EPP_DOMAIN);
            break;
        case EPP_TRANSFER_NSSET:
            cdata->noresdata = 1;
            cstat = epp_call_transfer(epp_ctx, service, loginid, request_id, cdata, EPP_NSSET);
            break;
        case EPP_TRANSFER_KEYSET:
            cdata->noresdata = 1;
            cstat = epp_call_transfer(epp_ctx, service, loginid, request_id, cdata, EPP_KEYSET);
            break;
        case EPP_SENDAUTHINFO_DOMAIN:
            cdata->noresdata = 1;
            cstat = epp_call_sendauthinfo(epp_ctx, service, loginid, request_id, cdata, EPP_DOMAIN);
            break;
        case EPP_SENDAUTHINFO_CONTACT:
            cdata->noresdata = 1;
            cstat = epp_call_sendauthinfo(
                    epp_ctx, service, loginid, request_id, cdata, EPP_CONTACT);
            break;
        case EPP_SENDAUTHINFO_NSSET:
            cdata->noresdata = 1;
            cstat = epp_call_sendauthinfo(epp_ctx, service, loginid, request_id, cdata, EPP_NSSET);
            break;
        case EPP_SENDAUTHINFO_KEYSET:
            cdata->noresdata = 1;
            cstat = epp_call_sendauthinfo(epp_ctx, service, loginid, request_id, cdata, EPP_KEYSET);
            break;

        case EPP_CREDITINFO:
            cstat = epp_call_creditinfo(epp_ctx, service, loginid, request_id, cdata);
            break;
        case EPP_TEST_NSSET:
            cdata->noresdata = 1;
            cstat = epp_call_test_nsset(epp_ctx, service, loginid, request_id, cdata);
            break;
        case EPP_INFO_LIST_CONTACTS:
            cstat = epp_call_info(
                    epp_ctx, service, loginid, request_id, cdata, ccReg_IT_LIST_CONTACTS);
            break;
        case EPP_INFO_LIST_DOMAINS:
            cstat = epp_call_info(
                    epp_ctx, service, loginid, request_id, cdata, ccReg_IT_LIST_DOMAINS);
            break;
        case EPP_INFO_LIST_NSSETS:
            cstat = epp_call_info(
                    epp_ctx, service, loginid, request_id, cdata, ccReg_IT_LIST_NSSETS);
            break;
        case EPP_INFO_LIST_KEYSETS:
            cstat = epp_call_info(
                    epp_ctx, service, loginid, request_id, cdata, ccReg_IT_LIST_KEYSETS);
            break;
        case EPP_INFO_DOMAINS_BY_NSSET:
            cstat = epp_call_info(
                    epp_ctx, service, loginid, request_id, cdata, ccReg_IT_DOMAINS_BY_NSSET);
            break;
        case EPP_INFO_DOMAINS_BY_KEYSET:
            cstat = epp_call_info(
                    epp_ctx, service, loginid, request_id, cdata, ccReg_IT_DOMAINS_BY_KEYSET);
            break;
        case EPP_INFO_DOMAINS_BY_CONTACT:
            cstat = epp_call_info(
                    epp_ctx, service, loginid, request_id, cdata, ccReg_IT_DOMAINS_BY_CONTACT);
            break;
        case EPP_INFO_NSSETS_BY_CONTACT:
            cstat = epp_call_info(
                    epp_ctx, service, loginid, request_id, cdata, ccReg_IT_NSSETS_BY_CONTACT);
            break;
        case EPP_INFO_KEYSETS_BY_CONTACT:
            cstat = epp_call_info(
                    epp_ctx, service, loginid, request_id, cdata, ccReg_IT_KEYSETS_BY_CONTACT);
            break;
        case EPP_INFO_NSSETS_BY_NS:
            cstat = epp_call_info(
                    epp_ctx, service, loginid, request_id, cdata, ccReg_IT_NSSETS_BY_NS);
            break;
        case EPP_INFO_GET_RESULTS:
            cstat = epp_call_getInfoResults(epp_ctx, service, loginid, request_id, cdata);
            break;
        default:
...
void epp_call_CloseSession(epp_context *epp_ctx, service_EPP service, unsigned long long loginid)
{
    ...
    if (raised_exception(ev))
    {
        epplog(epp_ctx,
               EPP_ERROR,
               "CORBA exception in sessionClosed: "
               "%s",
               ev->_id);
        CORBA_exception_free(ev);
        /* ignore error */
