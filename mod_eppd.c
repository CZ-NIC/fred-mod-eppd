...
void epplog(epp_context *epp_ctx, epp_loglevel level, const char *fmt, ...)
{
    ...
    {
        ap_log_cerror(
                APLOG_MARK, APLOG_ERR, rv, conn, "apr_global_mutex_lock(epp_log_lock) failed");
    }
...
static int epp_read_request(epp_context *epp_ctx, char **content, unsigned *bytes)
{
    ...
    status = ap_get_brigade(
            conn->input_filters, bb, AP_MODE_READBYTES, APR_BLOCK_READ, EPP_HEADER_LENGTH);
    if (status != APR_SUCCESS)
    {
        ...
        epplog(epp_ctx,
               ...
               "(%d - %s)",
               status,
               apr_strerror(status, err_msg, sizeof(err_msg)));
        return 2;
    }
    ...
    if (len != EPP_HEADER_LENGTH)
    {
        ...
        epplog(epp_ctx,
               EPP_ERROR,
               "4 bytes of EPP header were read "
    ...
    len = hbo_size;
    status = ap_get_brigade(conn->input_filters, bb, AP_MODE_READBYTES, APR_BLOCK_READ, len);
    if (status != APR_SUCCESS)
    {
        epplog(epp_ctx,
               EPP_ERROR,
               "Error when reading epp request's "
               "body (%d)",
               status);
        apr_brigade_destroy(bb);
        return 2;
    ...
    if (len != hbo_size)
    {
        epplog(epp_ctx,
               EPP_ERROR,
               "EPP request's length (%u bytes) is "
        ...
        corba_status *cstat)
{
    ...
    bzero(cert_md5, 80);
    pem = epp_ssl_lookup(pool, conn->base_server, conn, NULL, "SSL_CLIENT_CERT");
    if ((pem == NULL) || (*pem == '\0') || !get_md5(cert_md5, pem))
    {
        epplog(epp_ctx,
               EPP_ERROR,
               "Error when getting client's "
        ...
        if (cstat == CORBA_OK)
        {
            ...
            if (log_cstat == CORBA_ERROR || log_cstat == CORBA_REMOTE_ERROR)
            {
                ...
                if (errmsg[0] != '\0')
                {
                    epplog(epp_ctx,
                           EPP_ERROR,
                           "Fatal error when logging CreateSession: %s ",
                           errmsg);
                }
                else
                {
                ...
                if (logd_mandatory)
                {
                    if (loginid != 0)
                    {
                        epplog(epp_ctx,
                               EPP_ERROR,
                               "Terminating session because of logging failure.");
        ...
        void *schema, epp_lang lang, char **response, gen_status *gstat, qhead *valerr)
{
    ...
    *gstat = epp_gen_response(epp_ctx, validate, schema, lang, cdata, response, valerr);
    ...
    switch (*gstat)
    {
        ...
        case GEN_ESCHEMA:
            ...
            break;
        case GEN_NOT_VALID:
            epplog(epp_ctx, EPP_ERROR, "Generated response does not validate");
    ...
    *session_id_save = session_id = 0;
    ...
    while (1)
    {
        ...
        if (pstat > PARSER_HELLO)
        {
            ...
            {
                case PARSER_NOT_XML:
                    epplog(epp_ctx, EPP_WARNING, "Request is not XML");
        ...
        {
            act_log_entry_id = log_epp_command(
                    epp_ctx,
                    logger_service,
                    remote_ipaddr,
                    cdata->xml_in,
        ...
        else
        {
            ...
            epplog(epp_ctx,
                   EPP_DEBUG,
                   "after corba call command "
                   "saved login id is %lld, login id is %d",
                   *login_id_save,
                   login_id);
            ...
            if (cdata->rc >= 2000)
            {
                epplog(epp_ctx,
                       EPP_DEBUG,
                       "(epp-cmd %d) response code %d: sleeping for %d ms",
                       cdata->type,
                       cdata->rc,
                       sc->defer_err);
            ...
            if (logger_service != NULL && act_log_entry_id != 0)
            {
                        ...
                        act_log_entry_id).success)
                {
                    epplog(epp_ctx,
                           EPP_LOGD_ERRLVL,
                           "Could not log EPP command response in fred-logd");
                    // TODO cannot return error code - 2-phase commit should be used
                }
            }
...
static void *get_corba_service(epp_context *epp_ctx, const char *name)
{
    ...
    if (service == NULL)
    {
        epplog(epp_ctx,
               ...
               "Check mod_corba's configuration.",
               name);
        return NULL;
    }
...
static int epp_process_connection(conn_rec *c)
{
    ...
    if (EPPservice == NULL)
    {
        epplog(&epp_ctx,
               EPP_ERROR,
               "Could not obtain object reference "
               "for alias '%s'.",
               sc->object);
        return HTTP_INTERNAL_SERVER_ERROR;
    }
...
static int epp_postconfig_hook(apr_pool_t *p, apr_pool_t *plog, apr_pool_t *ptemp, server_rec *s)
{
    ...
    while (s != NULL)
    {
        ...
        if ((sc != NULL) && (sc->epp_enabled))
        {
            ...
            {
                ap_log_error(APLOG_MARK, APLOG_ERR, 0, s, "EPP Servername not configured");
                return HTTP_INTERNAL_SERVER_ERROR;
            }
            if (sc->schema == NULL)
            {
                ap_log_error(APLOG_MARK, APLOG_ERR, 0, s, "EPP schema not configured");
                return HTTP_INTERNAL_SERVER_ERROR;
            }
...
static const command_rec eppd_cmds[] = {
        AP_INIT_FLAG(
                "EPPprotocol", set_epp_protocol, NULL, RSRC_CONF,
                "Whether this server is serving the epp protocol"),
        AP_INIT_TAKE1(
                "EPPschema", set_schema, NULL, RSRC_CONF, "URL of XML schema of EPP protocol"),
        AP_INIT_TAKE1(
                "EPPservername", set_servername, NULL, RSRC_CONF,
                "Name of server sent in EPP greeting"),
        AP_INIT_TAKE1(
                "EPPlog", set_epplog, NULL, RSRC_CONF,
                "The file where come all log messages from mod_eppd"),
        AP_INIT_TAKE1(
                "EPPloglevel", set_loglevel, NULL, RSRC_CONF,
                "Log level setting for epp log (fatal, error, warning, "
                "info, debug)"),
        AP_INIT_FLAG(
                "EPPlogdMandatory", set_epp_logd_mandatory, NULL, RSRC_CONF,
                "Whether fred-logd failure is fatal to EPP"),
        AP_INIT_FLAG(
                "EPPvalidResponse", set_valid_resp, NULL, RSRC_CONF,
        ...
        AP_INIT_TAKE1(
                "EPPlogdObject", set_logger_object, NULL, RSRC_CONF,
                "Alias under which is the reference to Logger object "
                "exported from mod_corba module. Default is \"\"."),
        AP_INIT_TAKE1(
                "EPPdeferErrors", set_defer_errors, NULL, RSRC_CONF,
...
static void register_hooks(apr_pool_t *p)
{
    ...
    ap_hook_process_connection(epp_process_connection, aszPre, NULL, APR_HOOK_MIDDLE);

    /* register epp filters */
    ap_register_output_filter("EPP_OUTPUT_FILTER", epp_output_filter, NULL, AP_FTYPE_CONNECTION);
}
